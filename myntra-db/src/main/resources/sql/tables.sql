...
CREATE TABLE BRANDS(id SERIAL,uuid VARCHAR(200) UNIQUE NOT NULL, brand_name VARCHAR(50) NOT NULL, customer_rating DECIMAL NOT NULL, number_of_customers_rated INTEGER NOT NULL DEFAULT 0, address_id INTEGER NOT NULL , PRIMARY KEY(id), FOREIGN KEY (address_id) REFERENCES ADDRESS(id) ON DELETE CASCADE);
...
DROP TABLE IF EXISTS BRANDS_ITEM CASCADE;
CREATE TABLE BRANDS_ITEM(id SERIAL,item_id INTEGER NOT NULL, brand_id INTEGER NOT NULL, PRIMARY KEY (id),FOREIGN KEY (item_id) REFERENCES item(id) ON DELETE CASCADE, FOREIGN KEY (brand_id) REFERENCES BRANDS(id) ON DELETE CASCADE);

DROP TABLE IF EXISTS BRANDS_CATEGORY CASCADE;
CREATE TABLE BRANDS_CATEGORY(id SERIAL,brand_id INTEGER NOT NULL, category_id INTEGER NOT NULL,PRIMARY KEY (id), FOREIGN KEY (category_id) REFERENCES CATEGORY(id) ON DELETE CASCADE, FOREIGN KEY (brand_id) REFERENCES BRANDS(id) ON DELETE CASCADE);
